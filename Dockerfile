...
RUN mkdir -p /usr/src/cepredenac
...
COPY . /usr/src/cepredenac/
WORKDIR /usr/src/cepredenac
...
RUN chmod +x /usr/src/cepredenac/tasks.py \
    && chmod +x /usr/src/cepredenac/entrypoint.sh
...
ENTRYPOINT /usr/src/cepredenac/entrypoint.sh
